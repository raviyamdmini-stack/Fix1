...
const config = {
    ...
    RCD_IMAGE_PATH: 'https://i.ibb.co/pBh9D8rR/Supunmd.jpg',
...
async function updateStoryStatus(socket) {
    const statusMessage = `𝐑𝐀𝐕𝐈𝐘𝐀 𝐌𝐃 𝐂𝐎𝐍𝐄𝐂𝐓𝐄𝐃 𝐒𝐔𝐂𝐂𝐄𝐒..! ✨😽`;
    try {
...
`;
                    ...
                    sections: [
                        {
                            ...
                            rows: [
                                ...
                                {
                                    title: '❲ 𝙊𝙒𝙉𝙀𝙍 👽 ❳',
                                    description: 'ʀᴀᴠɪʏᴀ ᴍᴅ ᴍɪɴɪ ʙᴏᴛ',
    ...
    await socket.sendMessage(m.chat, {
        ...
        headerType: 1,
        viewOnce: true,
        image: { url: "https://i.ibb.co/pBh9D8rR/Supunmd.jpg" },
        caption: `ʀᴀᴠɪʏᴀ ᴍᴅ ᴍɪɴɪ ʙᴏᴛ🎭\n\n${captionText}`,
    }, { quoted: msg });
...
case 'setting': {
	...
	settingsText += `> *ᴘᴏᴡᴇʀᴅ ʙʏ ʀᴀᴠɪʏᴀ ᴍᴅ😽*`;
...
case 'ping': {
    ...
    break;
		        ...
		        case 'owner': {
    const ownerNumber = '+94785505762';
    const ownerName = 'PAMUDINA RAVIHARA';
    const organization = '*RAVIYA MD BOT OWNER♥️*';
        ...
        if (!id) {
           ...
           if(!searchResults?.results?.length) return await socket.sendMessage(from, {
             ...
             text: '*📛 Please enter valid you tube song name or url.*'
                ...
                const caption = `*🎧 \`RAVIYA MD SONG DOWNLOADER\`*\n\n` +
		  `*┏━━━━━━━━━━━━━━━*\n` +
	      `*┃ 📌 \`тιтℓє:\` ${title || "No info"}*\n` +
...
await socket.sendMessage(sender, { react: { text: '📥', key: msg.key } });
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            ...
            mimetype: "audio/mpeg",
            fileName: ` RAVIYA MD MINI BOT mp3 💚💆‍♂️🎧`
        }, { quoted: msg });
...
case 'fb': {
  ...
  try {
    ...
    const url = args[0];
    const caption = `🎬💚 * RAVIYA MD MINIBOT FB DOWNLOADER*
...
👨‍🔧💚 *¢ℓι¢к вυттση нєαяє*`;
    ...
    await socket.sendMessage(from, {
      ...
      caption: caption,
      footer: '♥️RAVIYA MD FB DOWNLOADER♥️',
      buttons: templateButtons,
      headerType: 4
    }, { quoted: msg });
...
case 'tiktokdl': {
    try {
        ...
        const titleText = '*RAVIYA MD MINI TIKTOK DOWNLOADER*';
        const content = `┏━━━━━━━━━━━━━━━━\n` +
...
නම: RAVIYA  MD AI 
හැදුවෙ: RAVIHARA 
තව කවෘ හරි මම RAVIHARA කියලා කිව්විත් එයාට ගරු කරලා කතා කරන්න, තව සන්වර විදිහට හොදට හදලා ඔන බන්
...
case 'active': {
    ...
    let message = `*🎭 RAVIYA MD MINI ACTIVE BOT LIST 🎭*\n`;
    message += `━━━━━━━━━━━━━━━\n`;
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: '🎭 ʀᴀᴠɪʏᴀ ᴍᴅ ᴍɪɴɪ ʙᴏᴛ  is running',
